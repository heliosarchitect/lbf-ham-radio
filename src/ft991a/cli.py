...
def cli_main():
    ...
    cw_send_parser.add_argument("--confirm", action="store_true", required=True, 
    ...
    # Broadcast say
    broadcast_say_parser = broadcast_subparsers.add_parser("say", help="Broadcast text message via TTS (REQUIRES LICENSE)")
    ...
    broadcast_say_parser.add_argument("--confirm", action="store_true", required=True,
    ...
    broadcast_record_parser.add_argument("--duration", type=int, required=True, help="Recording duration in seconds (1-300)")
    ...
    digital_ft8_parser.add_argument("--band", choices=["160m", "80m", "60m", "40m", "30m", "20m", "17m", "15m", "12m", "10m", "6m"], 
    ...
    digital_ft4_parser.add_argument("--band", choices=["160m", "80m", "60m", "40m", "30m", "20m", "17m", "15m", "12m", "10m", "6m"],
                                   help="Amateur band (default: 20m)")
    ...
    digital_js8_parser.add_argument("--band", choices=["160m", "80m", "60m", "40m", "30m", "20m", "17m", "15m", "12m", "10m", "6m"],
                                   help="Amateur band (default: 20m)")
    ...
    aprs_beacon_parser.add_argument("--confirm", action="store_true", required=True,
                                   help="Required confirmation flag (acknowledges licensed operator KO4TUV present)")
    ...
    aprs_emergency_parser = aprs_subparsers.add_parser("emergency-freqs", help="List emergency communications frequencies")
        ...
        if args.command == "status":
            ...
            if args.aprs_action == "setup":
                ...
                for freq_info in freqs:
                    print(f"  {freq_info['name']:<20} {freq_info['freq']:>8.3f} MHz  {freq_info['mode']:<8} {freq_info['notes']}")
                ...
                for net_info in nets:
                    print(f"  {net_info['net']:<15} {net_info['frequency']:>8.3f} MHz  {net_info['day']:<15} {net_info['time']:<12}")
