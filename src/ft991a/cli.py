...
def cli_main():
    ...
    scan_band_parser.add_argument(
        "--window-clock",
        action="store_true",
        help="Show wall-clock schedule projection anchored to now from the window timeline",
    )
    scan_band_parser.add_argument(
        "--window-now",
        action="store_true",
    ...
    scan_band_parser.add_argument(
        "--window-brief",
        action="store_true",
        help="Show compact now + upcoming hotspot handoff brief for manual RX operations",
    )
    scan_band_parser.add_argument(
        "--window-cue",
        action="store_true",
    ...
    scan_band_parser.add_argument(
        "--window-snapshot",
        action="store_true",
        help="Show one-line machine-friendly handoff snapshot (key=value fields)",
    )
    scan_band_parser.add_argument(
        "--window-fingerprint",
        action="store_true",
    ...
    scan_band_parser.add_argument(
        "--window-route",
        action="store_true",
        help="Show route directive (MONITOR/PREP_HANDOFF/IMMEDIATE_HANDOFF) from snapshot state",
    )
    scan_band_parser.add_argument(
        "--window-control",
        action="store_true",
        help="Show consolidated machine control packet (snapshot+fingerprint+stability+route)",
    )
    scan_band_parser.add_argument(
        "--action-ready-ms",
        type=int,
            ...
            if args.scan_action == "band":
                ...
                if results:
                    ...
                    if (
                        ...
                        or args.window_route
                        or args.window_control
                    ):
                    ...
                    if (
                        ...
                        if args.window_clock:
                            print()
                            print(scanner.format_hotspot_window_clock(clock_steps))
                        ...
                        if args.window_brief:
                            print()
                            print(
                                scanner.format_hotspot_window_brief(
                                    scanner.build_hotspot_window_brief(
                                        clock_steps,
                        ...
                        if args.window_snapshot:
                            print()
                            print(
                                scanner.format_hotspot_window_snapshot(
                                    scanner.build_hotspot_window_snapshot(
                                        clock_steps,
                                        ready_threshold_ms=max(0, args.action_ready_ms),
                                        critical_threshold_ms=max(
                                            0, args.action_critical_ms
                        ...
                        if args.window_route:
                            ...
                            print(
                                scanner.format_hotspot_window_route(
                                    scanner.build_hotspot_window_route(snapshot)
                                )
                            )

                        if args.window_control:
                            print()
                            snapshot = scanner.build_hotspot_window_snapshot(
                                clock_steps,
                                ready_threshold_ms=max(0, args.action_ready_ms),
                            ...
                            print(
                                scanner.format_hotspot_window_control_packet(
                                    scanner.build_hotspot_window_control_packet(snapshot)
                                )
                            )
                else:
