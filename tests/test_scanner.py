...
class TestBandScanner:
    ...
    def test_build_hotspot_window_cue(self, scanner):
        """Cue builder should emit single-line state payload from live schedule."""
        clock = [
            HotspotWindowClockStep(1, 14003999, 18000, 1_700_000_000_000, 1_700_000_018_000, 1_700_000_030_000),
            HotspotWindowClockStep(2, 14012999, 12000, 1_700_000_018_000, 1_700_000_030_000, 1_700_000_030_000),
        ]

        cue = scanner.build_hotspot_window_cue(clock, now_epoch_ms=1_700_000_005_000)

        assert isinstance(cue, HotspotWindowCue)
        assert cue.active_rank == 1
        assert cue.next_rank == 2
        assert cue.ms_until_switch == 13000

    def test_format_hotspot_window_cue(self, scanner):
        """Cue formatter should provide concise active→next operator handoff line."""
        cue = HotspotWindowCue(
            generated_epoch_ms=1_700_000_005_000,
            active_rank=1,
            active_center_hz=14003999,
            next_rank=2,
        ...
        rendered = scanner.format_hotspot_window_cue(cue)

        assert rendered.startswith("Hotspot Window Cue:")
        assert "P1" in rendered
        assert "P2" in rendered
    ...
    def test_build_hotspot_window_decision(self, scanner):
        """Decision builder should map action countdown into urgency/recheck guidance."""
        clock = [
            HotspotWindowClockStep(1, 14003999, 18000, 1_700_000_000_000, 1_700_000_018_000, 1_700_000_030_000),
            HotspotWindowClockStep(2, 14012999, 12000, 1_700_000_018_000, 1_700_000_030_000, 1_700_000_030_000),
        ]

        low = scanner.build_hotspot_window_decision(
            clock,
            now_epoch_ms=1_700_000_005_000,
            ready_threshold_ms=5000,
            critical_threshold_ms=2500,
        )
        high = scanner.build_hotspot_window_decision(
            clock,
            now_epoch_ms=1_700_000_016_000,
            ready_threshold_ms=5000,
            critical_threshold_ms=2500,
        )
        critical = scanner.build_hotspot_window_decision(
            clock,
            now_epoch_ms=1_700_000_017_900,
            ready_threshold_ms=5000,
            critical_threshold_ms=2500,
        ...
        assert isinstance(low, HotspotWindowDecision)
        assert low.urgency == "LOW"
        assert low.recommended_check_ms == 5000
        assert isinstance(high, HotspotWindowDecision)
        assert high.urgency == "HIGH"
        assert isinstance(critical, HotspotWindowDecision)
        assert critical.urgency == "CRITICAL"
        assert critical.action == "SWITCH"

    def test_format_hotspot_window_decision(self, scanner):
        """Decision formatter should include action, urgency, and recheck cadence."""
        decision = HotspotWindowDecision(
            generated_epoch_ms=1_700_000_005_000,
            active_rank=1,
            active_center_hz=14003999,
            next_rank=2,
            ...
            decision_reason="handoff inside 2500 ms critical window",
        )

        rendered = scanner.format_hotspot_window_decision(decision)

        assert rendered.startswith("Hotspot Window Decision:")
    ...
    def test_build_and_format_hotspot_window_fingerprint(self, scanner):
        snapshot = HotspotWindowSnapshot(
            generated_epoch_ms=1_700_000_016_500,
            action="READY",
            urgency="HIGH",
        ...
        fp = scanner.build_hotspot_window_fingerprint(snapshot)
        assert isinstance(fp, HotspotWindowFingerprint)
        assert len(fp.signature) == 12
        rendered = scanner.format_hotspot_window_fingerprint(fp)
        assert "sig=" in rendered

    def test_build_and_format_hotspot_window_stability(self, scanner):
        snapshot = HotspotWindowSnapshot(
            generated_epoch_ms=1_700_000_016_500,
            action="SWITCH",
            urgency="CRITICAL",
        ...
        stability = scanner.build_hotspot_window_stability(snapshot)
        assert isinstance(stability, HotspotWindowStability)
